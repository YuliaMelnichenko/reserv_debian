...
`stage-deploy.yml` пока содержит ручную заглушку **Stage deployment (not enabled)**:
она только сообщает, что выкат ещё не включён. Закомментированный шаблон сохранён
ниже для последующей настройки `scripts/deploy-stage-release.sh`. Оставлять весь
YAML-файл закомментированным нельзя: Gitea воспринимает его как пустой документ
и может сообщать об ошибке `EOF`.
...
фильтр workflow. Он может отображаться под именем файла. Не запускайте заглушку
`stage-deploy.yml` вместо проверки runner.
