...
function take_changes_time ($userID) {
  mysql_query( 'SET NAMES utf8' );
  $changes = mysql_query("SELECT changes FROM visiting WHERE user_id = '$userID'");
  $row2 = mysql_fetch_array( $changes, MYSQL_ASSOC );
...
$Val = $Val2 + $Val4 - $Val3 + $Val9;              
      ...
      $content .=                  "<h5 class=\"middle\"> ".format_time_d_hhmmss_pure_HH( $Val )." - Норма часов</h5></br>";
      $Val = (int)($results[2]);
      $fact_word = "<h5 class=\"middle\"> - Факт </h5>";     
      $content .=                  format_time_d_hhmmss_pure_styled( $Val )."(".$ValRedmine.")"." $fact_word";
      $content .=                "</div>";
      $content .=        "</div>";
