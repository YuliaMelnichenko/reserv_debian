...
$__login = mysql_real_escape_string($_POST['login']);   	       
$__passwd = md5(md5(trim(mysql_real_escape_string($_POST['passwd'])))); 

$query = mysql_query("SELECT id, rate, defaultStartTime, allowedDelayMinutes, userTimeZoneMins, dayTransitionTime, remoteWork FROM employees 
                      WHERE login='$__login' and passwd = '$__passwd'"); 
$merr=mysql_error();
if ( !$query ) 
{
