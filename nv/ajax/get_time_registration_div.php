...
///// ////***
  ...
  //  $content .= "<td class=\"nopadding_s\" title=\"зеленый - приход до начала р.д. включительно\nжелтый - в промежуток от 10:00:01 до 10:30:00\nкрасный - после 10:30:00\" bgcolor=\"$bgcolor\" width = \"$timeRestributionValWidth\" align = \"center\">";
    if ( $isThereDelay == 2 )
    { 
      if ( $crossDay == 1 )
      { 
...
//$query = mysql_query("SELECT * FROM visiting a where a.user_id = '$userID' and a.state != 0 and a.in_dt > ADDDATE( '$currentDate', INTERVAL -1 DAY )");
$query = mysql_query("SELECT * FROM visiting a where a.user_id = '$userID' and a.in_dt >= '$startDTStr' and a.in_dt < '$stopDTStr'");
...
//echo $durations[2];
  ...
  echo "<table width=616 cellpadding=\"0\" cellspacing=\"0\" border=0>";
    echo "<tr>";
      echo "<td class=\"nopadding_s\" align=\"left\" width=50% style=\"font-size: 100%; margin:0; padding:0; margin-left:0;\">";
        echo "<button title=\"возврат состояния регистрации времени до предыдущего\" style=\"font-size: 100%; width:40px; height:20px; background-color:#f8d888; border:1px solid #888888;\" onclick=\"rollback_state(); location.reload();\"><img src=\"img/rollbackState.png\"></button>";
      echo "</td>";
      echo "<td class=\"nopadding_s\" align=\"right\" width=50% style=\"font-size: 100%; margin:0; padding:0; margin-left:0;\">";
        ...
        {
          echo "<button disabled id=\"pauseBtn\" title=\"в обеденное время и при отметке об уходе с рабочего места приостановка учета времени запрещена!\" style=\"font-size: 100%; width:40px; height:20px; background-color:#f8d888; border:1px solid #888888;\"><img src=\"img/pauseDisabled.png\"></button>";
        }
        else
        {
          echo "<button id=\"pauseBtn\" title=\"приостановка учета времени\" style=\"font-size: 100%; width:40px; height:20px; background-color:#f8d888; border:1px solid #888888;\" onclick=\"set_pause_header();\"><img src=\"img/pause.png\"></button>";
        }
      echo "</td>";
    echo "</tr>";
  echo "</table>";   
  ...
  if ( $state == 2 )
  {
    ...
    $timeManagement .= "<button style=\"font-size: 110%; width:".$btnWidth."px; height:".$btnHeight."px; background-color:#f8d888; border:1px solid #888888;\" onclick=\"reg_eat_start();\">Зарегистрировать время ухода на обед</button>";
    $timeManagement .= "</td>";
  ...
  if ( $state == 3 )
  {
    $timeManagement .= "<td class=\"nopadding_s\" height=10></td></tr><tr>";
    $timeManagement .= "<td class=\"nopadding_s\" style=\"font-size: 100%; margin:0; padding:0; margin-left:0;\">";
      $timeManagement .= "<button style=\"font-size: 100%; width:".$btnWidth."px; height:".$btnHeight."px; background-color:#f8d888; border:1px solid #888888;\" onclick=\"reg_eat_stop();\">Зарегистрировать время прихода с обеда</button>";
    $timeManagement .= "</td>";
  ...
  {  
    $timeManagement .= "<td class=\"nopadding_s\" height=10></td></tr><tr>";
    $timeManagement .= "<td class=\"nopadding_s\" style=\"font-size: 100%; margin:0; padding:0; margin-left:0;\">";
      $timeManagement .= "<button style=\"font-size: 100%; width:".$btnWidth."px; height:".$btnHeight."px; background-color:#f8d888; border:1px solid #888888;\" onclick=\"reg_out_work();\">Зарегистрировать время ухода с рабочего места</button>";
    $timeManagement .= "</td>";
