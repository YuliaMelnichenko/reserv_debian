<?php
header("Content-type: text/plain; charset=utf-8");
header("Cache-Control: no-store, no-cache, must-revalidate");
header("Cache-Control: post-check=0, pre-check=0", false);

session_start();

$userID = $_SESSION['ss_id']; 
$currentDate = date('Y-m-d');

include_once "/var/www/tori/nv/funcs.php";
include_once "/var/www/tori/nv/php_tori/connect.php";

mysqli_set_charset($link, "utf8");

error_reporting(E_ALL | E_STRICT) ;
ini_set('display_errors', 'On');

//$query = mysql_query("select ID, SUIR, STARTTIME, DESCRIPTION from ADD_TIME where ID in ( select max(ID) from ADD_TIME where STARTDATE = '$currentDate' and USERID = '$userID')");
$query = mysqli_query($link, "select ID, SUIR, STARTTIME, DESCRIPTION from ADD_TIME where STARTDATE = '$currentDate' and USERID = '$userID' and PAUSE_MODE = 1");

$merr=mysqli_error($link);
if (!$query)
{
  echo "<br>mysql_error = $merr<br>";
}
else
{
  $vn=mysqli_num_rows($query);
  if ( $vn == 0 )
  {
    echo "0";
  } 
  else
  {
    if ( $row = mysqli_fetch_array($query, MYSQLI_ASSOC) )
    {  
      $id = $row["ID"];
      $suid = $row["SUIR"];
      $startTime = $row["STARTTIME"];
      $desk = $row["DESCRIPTION"];
      $SUName = get_superuser_name_by_id( $suid );
      $currentTime = date("H:i:s");
      $duration = strtotime( $currentTime ) - strtotime( $startTime );
      $durationStr = format_time_d_hhmmss_pure( $duration );

      echo "<table bgcolor=\"#FFFFFF\" id=\"pauseFullScreen\">";
        echo "<tr>";
          echo "<td align= \"center\" valign=\"middle\">";
            ///
            echo "<table border=\"1\" bgcolor=\"#ddeeff\">";  
              echo "<tr>";
                echo "<td align=\"left\" width = \"250\">";
                  echo "<h5 class=\"bigbig1\">Учет времени приостановлен</h5>";
                echo "</td>";
              echo "</tr>";
              echo "<tr>";
                echo "<td>";  

                  echo "<table  width=450 cellpadding=\"0\" cellspacing=\"0\" border=0 style=\"margin:0; padding:0; margin-left:0;\" >";  
                    echo "<tr>";
                      echo "<td valign=\"middle\" align=\"left\">";
                        echo "<h5 class=\"big\">время начала</h5>";
                      echo "</td>";
                      echo "<td valign=\"middle\" align=\"left\">";
                        echo "<h5 class=\"big\">$startTime</h5>";
                      echo "</td>";
                    echo "</tr>";
                    
                    echo "<tr bgcolor=\"#ffffff\">";
                      echo "<td valign=\"middle\" align=\"left\">";
                        echo "<h5 class=\"big\">длительность</h5>";
                      echo "</td>";
                      echo "<td valign=\"middle\" align=\"left\">";
                        echo "<h5 class=\"big\">$durationStr</h5>";
                      echo "</td>";
                    echo "</tr>";
                    
                    echo "<tr>";
                      echo "<td valign=\"middle\" align=\"left\">";
                        echo "<h5 class=\"big\">согласовано</h5>";
                      echo "</td>";
                      echo "<td valign=\"middle\" align=\"left\">";
                        echo "<h5 class=\"big\">$SUName</h5>";
                      echo "</td>";
                    echo "</tr>";
                    
                    echo "<tr bgcolor=\"#ffffff\">";
                      echo "<td valign=\"middle\" align=\"left\">";
                        echo "<h5 class=\"big\">комментарий</h5>";
                      echo "</td>";
                      echo "<td valign=\"middle\" align=\"left\">";
                        echo "<h5 class=\"big\">$desk</h5>";
                      echo "</td>";
                    echo "</tr>";
                  echo "</table>";

                echo "</td>";
              echo "</tr>";
              echo "<tr>";
                echo "<td valign=\"middle\" align=\"center\">";
                   echo "<button style=\"margin:0; padding:0; font-size: 100%; width:390px; height:30px; background-color:#f8d888; border:1px solid #888888;\" onclick=\"resume_from_pause( '$id' );\">Возобновить учет времени</button>";
                echo "</td>";
              echo "</tr>";
            echo "</table>"; 
            ///
          echo "</td>"; 
        echo "</tr>";
      echo "</table>";
    }
  }

  echo "<script type=\"text/javascript\" charset=\"utf-8\">"; 
  echo "set_pause_full_screen();";
  echo "window.onresize = function() { set_pause_full_screen(); }";
  echo "</script>";
}
?>


                                                                         